<template>
  <v-row>
    ...
    >
      ...
      >
        <v-img
          :src="`https://picsum.photos/500/300?image=${product.product.id * 5 + 10}`"
          :lazy-src="`https://picsum.photos/10/6?image=${product.product.id * 5 + 10}`"
          aspect-ratio="1"
        ...
        <v-row>
          <v-col class="pl-10">
            <v-row>
              <span class="product-name float-left"> {{ product.product.name }}</span>
            </v-row>
            <v-row>
              <span class="product-price">{{ product.productLowestPrice.price}}</span>
            </v-row>
          </v-col>
          <v-col>
...
export default {
  ...
  created: function() {
    ...
    {
      this.listProductInfos.push(productList[i].name,productListInfo[i].price);
    }
  }
};
</script>
