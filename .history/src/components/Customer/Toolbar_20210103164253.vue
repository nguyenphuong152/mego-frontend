      ...
      <v-col cols="2" class="d-flex justify-end">
        ...
        >
          <template v-slot:activator="{ on, attrs }">
            <v-btn icon v-bind="attrs" v-on="on" @mouseover="render()">
          ...
          <v-list class="text-start nav">
            ...
            <v-list-item v-show="getUser" @click="logout()">
        ...
        <v-btn icon>
          <LogoutIcon v-if="getUser" class="logo"></LogoutIcon>
        </v-btn>
      </v-col>
    </v-row>
  </v-toolbar>
...
export default {
  ...
  computed: {
    getUser() {
      if (this.$store.getters.getUser.name != undefined) {
